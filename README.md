...
```
Then make a [pull request](https://github.com/douglascdev/hashbot/pulls).
...
The bot takes arguments using CLI arguments, either using `go run main.go` to run straigth from source code or the binary in the [releases section](https://github.com/douglascdev/hashbot/releases) with `./hashbot-linux` (on linux, remember to `chmod +x hashbot-linux` first).
